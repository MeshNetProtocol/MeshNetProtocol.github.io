...
### 1.2 范围
- 包含：SIWE（EIP-4361）登录、供应商 API、前端接入、安全基线、可选链上增强方案。
...
## 6. 可选链上增强计划（对应方面 5：上链增强）

### 6.1 适用场景
- 需要公开可验证的供应商身份/状态。
- 需要第三方可独立验证供应商状态，不完全信任中心化后端。

### 6.2 方案选择
- MVP（推荐先做）：供应商身份与配置全部保存在后端，不上链。
- 增强版（二期）：把供应商注册与状态哈希上链，后端读取链事件进行校验或索引。

### 6.3 里程碑 E（可选二期）

#### 开发步骤
1. 定义最小合约结构（SupplierRegistered、SupplierStatusUpdated 事件）。
2. 部署到测试网（例如 Sepolia）并接入后端读取。
3. 在 `market-api` 增加链上状态校验开关（feature flag）。
4. 前端增加“链上已验证”状态展示。

#### 测试步骤
1. 合约单元测试：注册、更新、事件参数正确性。
2. 后端集成测试：链上事件到后端状态同步。
3. 端到端测试：前端展示与后端校验一致。

#### 验收标准
- 链上事件与后端索引数据一致率 100%（测试环境）。
- 开关关闭时系统行为与一期一致，不影响线上稳定性。
...
3. 里程碑 C：前端接入（4-6 天）
4. 里程碑 D：安全加固（2-4 天）
5. 里程碑 E：链上增强（可选，5-10 天）

> 建议先完成 A+B+C 形成可用闭环，再进入 D；E 仅在确有公开可验证需求时启动。
...
### 8.1 测试层级
- 单元测试：签名校验、权限函数、配置校验器。
- 集成测试：API + DB + 鉴权中间件。
- E2E 测试：浏览器 + MetaMask + `market-api`。

### 8.2 回归用例最小集
1. 新钱包首次登录并创建供应商成功。
2. owner 修改配置成功，重新读取一致。
3. 非 owner/manager 修改配置被拒绝。
4. 重放签名请求失败。
5. token 过期后受保护接口返回 401，重新登录恢复。

### 8.3 发布门禁（Go/No-Go）
- 所有 P0/P1 测试通过。
- 安全用例（重放、越权、跨域）全部通过。
- 关键接口错误率与延迟在阈值内（由团队定义）。
