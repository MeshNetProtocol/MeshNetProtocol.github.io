...
1. 用户可在控制台主动切换 `Base Mainnet` 与 `Base Sepolia`。
2. 切换后前端必须同步切换：
- 目标 `chainId`
- `ProtocolRegistry` 合约地址
- `USDC` 地址
- API 的网络上下文参数（若有）
3. 切换后必须自动重跑网络对齐与数据刷新流程；若当前会话与网络不一致，要求重新签名登录。
...
2. 能在 Base Mainnet 与 Base Sepolia 间切换，并自动切换对应合约与 USDC 配置。
3. 能完成签名登录并展示会话状态。
4. 能读取费用配置与我的商业供应商列表。
5. 能注册商业供应商（授权支付 + 上链）。
6. 能续费商业供应商（一年一续）。
...
8. 能转移 owner。
9. 能预估并执行提现。
10. 能导入私人供应商（文件 / URL）并本地更新。
11. 新增文案全部走 i18n，不硬编码中英文混排。
